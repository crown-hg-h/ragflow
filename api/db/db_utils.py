...
def bulk_insert_into_db(model, data_source, replace_on_conflict=False):
    ...
    def _fallback_upsert(rows):
        pk_names = getattr(model, "get_primary_keys_name", lambda: [model._meta.primary_key.name])()
        for row in rows:
            try:
                with DB.atomic():
                ...
                where_expr = None
                for pk in pk_names:
                    expr = getattr(model, pk) == row[pk]
                    where_expr = expr if where_expr is None else (where_expr & expr)
                update_fields = {k: v for k, v in row.items() if k in preserve}
                if where_expr is not None and update_fields:
                    with DB.atomic():
                        model.update(**update_fields).where(where_expr).execute()
    ...
    for i in range(0, len(data_source), batch_size):
        ...
        try:
            ...
            if "CONFLICT" in str(e).upper():
                try:
                    DB.rollback()
                except Exception:
                    pass
                _fallback_upsert(rows)
            else:
                raise
