...
try:
    ...
    if not REDIS:
        raise Exception("Redis config is empty")
except Exception as e:
    logging.warning(f"Failed to load Redis config: {e}, using default")
    # Fallback to default config if decryption fails
    REDIS = get_base_config("redis", {"host": "127.0.0.1:6379", "password": "", "db": 1})
