...
class RedisDB:
    ...
    def __open__(self):
        try:
            ...
            # Test connection
            self.REDIS.ping()
            logging.info(f"Redis connected successfully to {self.config['host']}")
        except Exception as e:
            logging.error(f"Redis connection failed: {e}")
            logging.error(f"Redis config: host={self.config.get('host', 'N/A')}, db={self.config.get('db', 'N/A')}")
            import traceback

            logging.error(f"Redis connection traceback: {traceback.format_exc()}")
            self.REDIS = None
        return self.REDIS
            ...
            try:
                ...
                def _json_safe(obj):
                    try:
                        ...
                        # Peewee Field or other non-serializable -> cast to str
                        if hasattr(obj, "__class__") and obj.__class__.__name__.endswith("Field"):
                            return str(obj)
                ...
                payload = {"message": json.dumps(_json_safe(message))}
                msg_id = self.REDIS.xadd(queue, payload)
                logging.info(f"RedisDB.queue_product xadd {queue} -> {msg_id}")
                return True
            except Exception as e:
